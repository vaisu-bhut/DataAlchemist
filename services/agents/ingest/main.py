    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                ...
                logger.error(
                    "❌ Failed to connect to Neo4j after 3 attempts - service will start but may not process requests"
                )
        ...
        while True:
            try:
                ...
                response = await client.get(
                    f"{PUBSUB_URL}/poll/ingest.request", timeout=30.0
                )
...
async def process_request(message: dict):
    ...
    try:
        ...
        conversations = [ConversationData(**conv) for conv in conversations_data]
        ...
        logger.error("Processing failed", correlation_id=correlation_id, error=str(e))
