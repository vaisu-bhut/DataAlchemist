...
class RetrievalService:
    ...
    async def retrieve_and_respond(self, query: str, customer_id: str, context: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    async def _retrieve_candidates(self, query_embedding: List[float], customer_id: str) -> List[Dict]:
        """Retrieve candidate issues and solutions using vector similarity"""
        
        logger.info(f"Retrieving candidates for query embedding of length {len(query_embedding)}")
        ...
        try:
            issue_results = await self.neo4j.execute_query(issue_query, {
        ...
        solution_results = await self.neo4j.execute_query(solution_query, {
            'embedding': query_embedding,
            'k': settings.max_retrieval_results,
            'limit': settings.max_retrieval_results
        })
            ...
            for solution in result.get('solutions', []):
                if solution.get('id'):  # Only include if solution exists
                    candidates.append({
                        ...
                        'quality_score': (result['issue_quality'] + solution['quality_score']) / 2,
                        'human_reviewed': result['issue_reviewed'] and solution['human_reviewed'],
    ...
    async def _rank_candidates(self, candidates: List[Dict], query: str) -> List[Dict]:
        ...
        for candidate in candidates:
            ...
            quality_score = candidate['quality_score'] * 0.3
            human_bonus = 0.2 if candidate['human_reviewed'] else 0.0
            
            # Simple recency bonus (would need timestamps for proper implementation)
            recency_bonus = 0.1
            
            candidate['composite_score'] = base_score + quality_score + human_bonus + recency_bonus
    ...
    async def _build_source_references(self, candidates: List[Dict], source_ids: List[str]) -> List[Dict]:
        ...
        for candidate in candidates:
            if candidate['source_id'] in source_ids:
                ...
                snippet_result = await self.neo4j.execute_query(snippet_query, {
